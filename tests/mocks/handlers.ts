...
export const handlers = [
  http.post<PathParams<string>, { password: string; username: string }>(
  ...
  http.get<{ id: string }>('https://fetestapi.int.mozzaik365.net/api/users/:id', async ({ params }) => {
  ...
  http.get<{ id: string }>('https://fetestapi.int.mozzaik365.net/api/memes/:id/comments', async ({ params }) => {
    const memeComments = comments.filter(comment => comment.memeId === params.id);
    return HttpResponse.json({
  ...
  http.post<{ id: string }, { content: string }>(
    ...
    async ({ params, request }) => {
      ...
      // Update the comments count of the meme
